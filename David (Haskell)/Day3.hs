...
housesVisited = S.size . snd . foldl go ((0,0), S.singleton (0,0))
  where go (pos,set) c = let pos' = move pos c 
                         in (pos', S.insert pos' set)
...
move (x,y) '<' = (x-1,y)
move (x,y) '^' = (x,y+1)
move _     _   = error "Bad input"

test1 = housesVisited ">" == 2
     && housesVisited "^>v<" == 4
     && housesVisited "^v^v^v^v^v" == 2
...
housesVisitedWithRoboSanta = S.size . (\(_,_,x) -> x) . foldl go start 
  where start = ((0,0),(0,0),S.singleton (0,0))
        go (pos1,pos2,set) c = let pos1' = move pos1 c
                               in (pos2,pos1',S.insert pos1' set)

test2 = housesVisitedWithRoboSanta "^v" == 3
     && housesVisitedWithRoboSanta "^>v<" == 3
     && housesVisitedWithRoboSanta "^v^v^v^v^v" == 11
