...
paperNeeded :: String -> Int
paperNeeded = sum . map paperPerBox . parse

paperNeededFromFile :: IO Int
paperNeededFromFile = paperNeeded <$> readFile "input"

paperPerBox :: (Int,Int,Int) -> Int
paperPerBox (l,w,h) = 2*(l*w + w*h + h*l) + minimum [l*w,w*h,h*l]

test1 = paperNeeded "2x3x4"  == 58
     && paperNeeded "1x1x10" == 43
...
ribbonNeeded :: String -> Int
ribbonNeeded = sum . map ribbonPerBox . parse

ribbonNeededFromFile :: IO Int
ribbonNeededFromFile = ribbonNeeded <$> readFile "input"

ribbonPerBox :: (Int,Int,Int) -> Int
ribbonPerBox (l,w,h) = 2*(a+b) + l*w*h
  where (a,b) = if l > w then (w,min l h) else (l, min w h)

test2 = ribbonNeeded "2x3x4" == 34
...
parse = map go . lines
  where go = (\[a,b,c] -> (a,b,c)) . map read . splitOn (=='x')
